...
{
	return (sfSprite_getTextureRect(_animation->sprite));
}
	...
	sfVector2u* _blockLength)
{
	...
	else
	{
		...
		sfSprite_setTextureRect(
			_anim->sprite,
			(sfIntRect) {
			0, 0, _anim->frameSize.x, _anim->frameSize.y
		});
	}
}
